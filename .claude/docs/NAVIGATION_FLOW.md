...
`maxBlocks`, `optimalBlocks`/`reasonText`+`hintText`) a partir de tipos
...
## Detalhe por tela
- **Splash/Menu**: logo, palco central revezando a Lili programando (`assets/images/mascot_coding.png`) e os ícones dos 7 mundos, botão Jogar pulsando, selo LiCode. Sem botão de configurações (saiu daqui, ver Seleção de Mundo abaixo — pedido explícito do usuário, ver `.claude/memory/decisions.md`). Sem lógica de jogo.
...
- **Tentativa Falha (Mundos 1 e 2, `FailureScreen`)**: motivo real da falha (Mundo 1: bateu na parede/saiu do tabuleiro vs. não chegou ao alvo; Mundo 2: item classificado na caixa errada vs. sobraram itens na esteira), mascote confuso, card de Dica com a frase escrita da fase (`Level.hintText`, nunca os blocos da solução). Som suave + vibração ao abrir (`AppSounds.instance.failure()`), deliberadamente não-punitivo. Genérica entre motores (ver acima).
