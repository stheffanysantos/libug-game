...
void main() {
  ...
  test('addSessionPoints soma à pontuação de sessão', () {
    ...
    expect(container.read(progressProvider).sessionScore, 150);
  });
  ...
  test('submitToLeaderboard marca hasSubmittedToLeaderboard e guarda idade/resposta', () {
    ...
    expect(container.read(progressProvider).hasSubmittedToLeaderboard, isFalse);
  ...
  test('markGameCompleted marca gameCompleted uma vez só, sem sobrescrever gameCompletedAt numa 2ª chamada', () async {
    ...
    final firstCompletedAt = container.read(progressProvider).gameCompletedAt;
    expect(container.read(progressProvider).gameCompleted, isTrue);
    expect(firstCompletedAt, isNotNull);

    await Future<void>.delayed(const Duration(milliseconds: 5));
    notifier.markGameCompleted();
    expect(container.read(progressProvider).gameCompletedAt, firstCompletedAt, reason: 'não deve reescrever a data de quem já tinha zerado');
  });

  test('markWelcomeSeen marca seenWelcome=true e sincroniza; idempotente numa 2ª chamada', () {
    ...
    expect(container.read(progressProvider).seenWelcome, isFalse);

    notifier.markWelcomeSeen();
    expect(container.read(progressProvider).seenWelcome, isTrue);
    expect(repo.lastSaved, isNotNull, reason: '_syncNow gravou o estado no repositório');
    expect(repo.lastSaved!.seenWelcome, isTrue);

    // 2ª chamada é no-op (`if (state.seenWelcome) return`) — não regrava.
    repo.lastSaved = null;
    notifier.markWelcomeSeen();
    expect(container.read(progressProvider).seenWelcome, isTrue);
    expect(repo.lastSaved, isNull, reason: 'idempotente — não sincroniza de novo se já estava true');
  });
  ...
  test('_hydrate mescla (não substitui) quando já há progresso local ao logar numa conta de outro aparelho', () async {
    ...
    expect(container.read(progressProvider).isCompleted('fase1'), isTrue);
  ...
  test('setUsername/setAvatarId guardam a escolha do jogador; displayAvatarId cai no padrão sem avatar escolhido', () {
    ...
    expect(container.read(progressProvider).username, isNull);
    expect(container.read(progressProvider).displayAvatarId, 'lili', reason: 'padrão do jogo antes de escolher');
