...
import '../gameplay/code_quest_gameplay_view.dart';
...
/// `predictOutputStageSelectRouteName`.
const codeQuestStageSelectRouteName = 'code-quest-stage-select';

/// Seleção de Fases do Mundo 4 ("Missão de Código") — mesmo papel de
/// `StageSelectView` (Mundo 1), adaptado para `CodeQuestLevel`/
/// `world4Levels`.
class CodeQuestStageSelectView extends ConsumerWidget {
  final GameWorld world;

  const CodeQuestStageSelectView({super.key, required this.world});

  List<CodeQuestLevel> get _levels => world.levels.cast<CodeQuestLevel>();
  ...
  void _openLevelAt(BuildContext context, int index) {
    Navigator.of(context).push(MaterialPageRoute(builder: (_) => CodeQuestGameplayView(levelId: _levels[index].id)));
  }
